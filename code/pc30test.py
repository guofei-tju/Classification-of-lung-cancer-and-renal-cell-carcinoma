...
for di in range(2):
    for it in range(3):
        ...
        print(name + ':')
        sample = read_file_np1('E:\multiple_classification\multi_classification\pca_30\\' + dis + '_' + name + '.csv')
        ...
        if dis == 'LUNG':
            ...
            wb = load_workbook("rf.xlsx")  # 生成一个已存在的wookbook对象
            ...
            # kidney五折交叉验证
            # scorerMCC = metrics.make_scorer(metrics.matthews_corrcoef)   二分类
            ...
            scorer = {'ACC': 'accuracy', 'recall': scorerSE, 'roc_auc': 'roc_auc_ovo', 'SP': scorerSP}
            five_fold = model_selection.cross_validate(rf, sample, label, cv=cv, scoring=scorer)
            print(five_fold)
            mean_ACC = np.mean(five_fold['test_ACC'])
            mean_sensitivity = np.mean(five_fold['test_recall'])
